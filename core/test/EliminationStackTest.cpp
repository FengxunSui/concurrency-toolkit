...
#include "lockfree/EliminationBackoffStack.h"
#include <atomic>
...
template <typename T>
std::shared_ptr<T> try_pop_for(EliminationBackoffStack<T> &stack,
...
TEST_CASE("EliminationBackoffStack: 单线程基本操作", "[stack][single_thread]") {
...
TEST_CASE("EliminationBackoffStack: 单生产者单消费者 (SPSC)",
...
TEST_CASE("EliminationBackoffStack: 多生产者多消费者压力测试 (MPMC)",
...
TEST_CASE("EliminationBackoffStack: 复杂数据类型支持", "[stack][complex_type]") {
  struct TestStruct {
    std::string str;
    int value;
  ...
  EliminationBackoffStack<TestStruct> stack;
...
TEST_CASE("EliminationBackoffStack: 内存安全与生命周期", "[stack][memory][stress]") {
...
TEST_CASE("EliminationBackoffStack: Hazard Pointer 槽位压力测试",
...
TEST_CASE("EliminationBackoffStack: 性能基准", "[stack][benchmark][!mayfail]") {
  EliminationBackoffStack<int> stack;
  ...
  std::cout << "\nElimination[Perf] " << iterations << " ops in " << duration << "ms"
...
TEST_CASE("EliminationBackoffStack: 微基准", "[!benchmark]") {
  EliminationBackoffStack<int> stack;
